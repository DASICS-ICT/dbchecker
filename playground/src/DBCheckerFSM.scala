  ...
  switch(r_chan_status) {
    ...
    is(DBCheckerState.WaitCheckresp) {
      ...
      when(decrypt_resp.valid && cur_decrypt_output_no === r_decrypt_no) {
        ...
        val bnd_err        =
          (r_araddr_ptr.access_addr < bnd_base) || ((r_araddr_ptr.access_addr + r_ar_reg.len) > (bnd_base + bnd_offset))
...
// write channel(aw/w/b)
    ...
    is(DBCheckerState.WaitCheckresp) {
      ...
      when(decrypt_resp.valid && cur_decrypt_output_no === w_decrypt_no) {
        ...
        when(magic_num_err || bnd_err || bnd_mismatch) { // magic_num check
          ...
          w_chk_err          := true.B
          err_req_w.valid    := true.B
          err_req_w.bits.typ := Mux(
            magic_num_err,
            err_mtdt_fmn,
            Mux(bnd_err, err_bnd_farea, err_bnd_ftype)
          ) // 1: mtdt error 0:bnd_error
  ...
  debug_if := Cat(
    r_chan_status.asUInt,
    w_chan_status.asUInt,
    r_decrypt_no,
    w_decrypt_no,
    cur_decrypt_input_no,
    cur_decrypt_output_no,
    w_chk_err.asUInt,
    r_chk_err.asUInt
  )
}
