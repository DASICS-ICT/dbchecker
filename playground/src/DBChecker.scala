...
// write channel(aw/w/b)
    ...
    is(DBCheckerState.ReadDBTE) {
      ...
      when (s_axi_io_rx.aw.valid) {
        ...
        .elsewhen (!ctrl.dbte_v_bm(dbte_index)) {
          ...
          ctrl.err_req_w.bits.info := target_addr
          ctrl.err_req_w.bits.mtdt := 0.U
  ...
  debug_if.flow := Cat(r_chan_status.asUInt, w_chan_status.asUInt, r_decrypt_no, w_decrypt_no, cur_decrypt_input_no, cur_decrypt_output_no, w_chk_err.asUInt, r_chk_err.asUInt)
}