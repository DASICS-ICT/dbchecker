...
object AxiMM2S {
  def apply(mm: AxiMaster, maxTxLen: Int = DefaultMaxTxLen) = {
    val mm2s = Module(new AxiMM2S(mm.ar.bits, mm.r.bits, maxTxLen))
    mm2s.io.mm.addr <> mm.ar
    mm2s.io.mm.data <> mm.r
    mm2s
  }
}
...
object AxiS2MM {
  def apply(mm: AxiMaster, maxTxLen: Int = DefaultMaxTxLen) = {
    val s2mm = Module(new AxiS2MM(mm.aw.bits, mm.w.bits, maxTxLen))
    s2mm.io.mm.addr <> mm.aw
    s2mm.io.mm.data <> mm.w
    mm.b.ready := true.B
    s2mm
  }
}